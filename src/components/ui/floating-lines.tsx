...
vec3 getLineColor(float t) {
  ...
  vec3 c2 = lineGradient[min(idx + 1, lineGradientCount - 1)];
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  if (enableBottom) {
    for (int i = 0; i < 20; ++i) {
      ...
      col += lineCol * wave(ruv + vec2(bottomLineDistance * float(i) + bottomWavePosition.x, bottomWavePosition.y), 1.5 + 0.2 * float(i), baseUv, mouseUv, interactive) * 0.2;
    }
  }
  ...
  if (enableMiddle) {
    for (int i = 0; i < 20; ++i) {
      ...
      vec2 ruv = baseUv * rotate(angle);
      col += lineCol * wave(ruv + vec2(middleLineDistance * float(i) + middleWavePosition.x, middleWavePosition.y), 2.0 + 0.15 * float(i), baseUv, mouseUv, interactive);
    }
  }
  ...
  if (enableTop) {
    for (int i = 0; i < 20; ++i) {
      ...
      vec2 ruv = baseUv * rotate(angle);
      ruv.x *= -1.0;
      col += lineCol * wave(ruv + vec2(topLineDistance * float(i) + topWavePosition.x, topWavePosition.y), 1.0 + 0.2 * float(i), baseUv, mouseUv, interactive) * 0.1;
    }
  }
...
export default function FloatingLines({
  ...
  useEffect(() => {
    ...
    const getVal = (v: any, idx: number, def: number) => Array.isArray(v) ? (v[idx] ?? def) : v;
    ...
    const uniforms = {
      ...
      topLineCount: { value: getVal(lineCount, 0, 6) },
      middleLineCount: { value: getVal(lineCount, 1, 6) },
      bottomLineCount: { value: getVal(lineCount, 2, 6) },
      topLineDistance: { value: getVal(lineDistance, 0, 5) * 0.01 },
      middleLineDistance: { value: getVal(lineDistance, 1, 5) * 0.01 },
      bottomLineDistance: { value: getVal(lineDistance, 2, 5) * 0.01 },
    ...
    const handlePointerMove = (e: MouseEvent) => {
      ...
      targetMouseRef.current.set((e.clientX - rect.left) * renderer.getPixelRatio(), (rect.height - (e.clientY - rect.top)) * renderer.getPixelRatio());
      targetInfluenceRef.current = 1.0;
      if (parallax) {
        targetParallaxRef.current.set(((e.clientX - rect.left) / rect.width - 0.5) * parallaxStrength, -((e.clientY - rect.top) / rect.height - 0.5) * parallaxStrength);
      }
    };

    if (interactive) {
      container.addEventListener('mousemove', handlePointerMove);
      container.addEventListener('mouseleave', () => targetInfluenceRef.current = 0);
    }
