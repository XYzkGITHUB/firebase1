...
// Lazy load PartnerShowcase
const PartnerShowcase = dynamic(() => import('./PartnerShowcase').then(mod => mod.PartnerShowcase), {
  ssr: false,
      ...
      >
        <div className="relative w-full h-full border border-primary/20 p-4">
          <div className="relative w-full h-full overflow-hidden" style={{ clipPath: "polygon(10% 0, 100% 0, 90% 100%, 0% 100%)" }}>
            {images.tiles && (
              <Image src={images.tiles.imageUrl} alt="Texture" fill className="object-cover grayscale brightness-125" loading="lazy" />
            )}
          </div>
        </div>
      </motion.div>
      ...
      >
        <div className="relative w-full h-full border border-primary/10 p-2">
          <div className="relative w-full h-full overflow-hidden" style={{ clipPath: "polygon(0 15%, 100% 0, 100% 85%, 0 100%)" }}>
            {images.laminate && (
              <Image src={images.laminate.imageUrl} alt="Texture 2" fill className="object-cover" loading="lazy" />
            )}
          </div>
        </div>
      </motion.div>
      ...
      <div className="max-w-[1400px] w-full mx-auto relative z-10 flex flex-col items-center">
        <div className="text-center mb-20">
          <div className="mb-6 h-auto min-h-[120px] flex items-center justify-center">
            <TrueFocus sentence="Быстрая навигация IRGG" manualMode={false} blurAmount={5} borderColor="hsl(var(--primary))" glowColor="hsl(var(--primary) / 0.5)" animationDuration={0.4} pauseBetweenAnimations={1.5} className="text-5xl md:text-8xl font-headline uppercase tracking-tighter" />
          </div>
          <p className="text-muted-foreground text-lg md:text-xl font-light uppercase tracking-widest max-w-2xl mx-auto leading-relaxed">Выберите интересующий раздел</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative w-full">
          ...
          >
             <div className="absolute inset-0 opacity-20 transition-transform duration-1000 group-hover:scale-110 pointer-events-none">
               {images.showroom && (
                 <Image src={images.showroom.imageUrl} alt="Store Preview" fill className="object-cover mix-blend-overlay" loading="lazy" />
               )}
             </div>
             <div className="relative z-10">
               <div className="w-12 h-12 bg-white/10 flex items-center justify-center mb-8"><ShoppingBag size={24} className="text-white" /></div>
